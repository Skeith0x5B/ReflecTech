...
*/
#ifndef RT_DOUBLY_LINKED_LIST_DP_V_H
#define RT_DOUBLY_LINKED_LIST_DP_V_H
...
public:
	...
	{
	public:
		// ctors & dtor - no copy ctor, don't create too many potential points through which to modify data
	...
	// ctors/dtor				   
	DoublyLinkedList(bool _isAligned = false, U8 _alignment = 1);
...
private:	
	// private clear - this will destroy the head and tail, used the dtor before freeing memory
	void InternalClear();
...
/**************************************************************************************************************************/
template<class T>
DoublyLinkedList<T>::DoublyLinkedList(bool _isAligned, U8 _alignment)
{
	...
	// and declare it static so it won't be destroyed upon leaving the ctor, we then set the pointer to it to circumvent scope issues
...
#endif // RT_DOUBLY_LINKED_LIST_DP_V_H
